...
-- 1) Aba "Em Fila" inclui conversas com IA atendendo (bot_active)
...
-- 2) Contadores: "Atendendo" = humano, "Em Fila" = waiting + bot
...
-- 3) initialize_user_permissions: vendedor volta a ter view_queue_conversations = true
...
-- 4) Backfill: vendedores recuperam acesso à fila do seu setor
UPDATE public.user_permissions up
SET view_queue_conversations = true
WHERE view_queue_conversations = false
  AND EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = up.user_id AND ur.role = 'seller'::app_role
  );
