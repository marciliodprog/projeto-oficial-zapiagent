-- Drop versões antigas para mudar return type
DROP FUNCTION IF EXISTS public.inbox_count_conversations(uuid,uuid[],boolean,uuid[],boolean,uuid[],boolean,uuid[],text,text,text[],uuid[],uuid[],uuid[]);
DROP FUNCTION IF EXISTS public.inbox_count_conversations(uuid,uuid[],boolean,uuid[],boolean,uuid[],boolean,uuid[],text,text);
DROP FUNCTION IF EXISTS public.inbox_list_conversations(uuid,text,uuid[],boolean,uuid[],boolean,uuid[],boolean,uuid[],text,text,timestamp with time zone,integer,text[],uuid[],uuid[],uuid[]);
...
-- LIST: adiciona aba 'agents' e separa attending/waiting de bot_active
...
RETURNS TABLE(
  ...
  status text, channel text, needs_human boolean,
...
GRANT EXECUTE ON FUNCTION public.inbox_list_conversations(uuid,text,uuid[],boolean,uuid[],boolean,uuid[],boolean,uuid[],text,text,timestamp with time zone,integer,text[],uuid[],uuid[],uuid[]) TO authenticated, service_role;


-- COUNT: agora retorna attending/agents/waiting/resolved
...
BEGIN
  ...
  RETURN QUERY
  WITH base AS (
    SELECT c.status, c.current_agent_id, c.id
    FROM public.webchat_conversations c
    LEFT JOIN public.leads l           ON l.id = c.lead_id
    LEFT JOIN public.webchat_widgets w ON w.id = c.widget_id
    WHERE
...
-- Limpa funções placeholder de 10 params criadas anteriormente
DROP FUNCTION IF EXISTS public.inbox_list_conversations(uuid,text,uuid[],boolean,uuid[],boolean,uuid[],boolean,uuid[],text,text,timestamp with time zone,integer);
DROP FUNCTION IF EXISTS public.inbox_count_conversations(uuid,uuid[],boolean,uuid[],boolean,uuid[],boolean,uuid[],text,text);